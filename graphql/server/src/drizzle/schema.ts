import { pgTable, pgPolicy, integer, text, foreignKey } from "drizzle-orm/pg-core"
...
export const prefecture = pgTable("prefecture", {
	...
	pgPolicy("prefecture_policy", { as: "permissive", for: "all", to: ["authenticated", "postgres"], using: sql`true` }),
]);
...
export const municipality = pgTable("municipality", {
	...
	pgPolicy("municipality_policy", { as: "permissive", for: "all", to: ["authenticated", "postgres"], using: sql`true` }),
]);
